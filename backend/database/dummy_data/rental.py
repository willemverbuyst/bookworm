...
def create_list(number_of_rentals, start, end):
    ...
    return (
        formatted_dates_mondays.to_list()
        + formatted_dates_tuesdays.to_list()
        + formatted_dates_wednesdays.to_list()
        + formatted_dates_thursdays.to_list()
        + formatted_dates_fridays.to_list()
        + formatted_dates_saturdays.to_list()
        + formatted_dates_sundays.to_list()
    )
...
def create_dummy_rentals_sql(config):
    ...
    insert_statements = ""
    old_rentals = create_list(
        len(config.get("RENTAL")[400:]), "2000-01-01", "2023-01-31"
    )
    new_rentals = create_list(
        len(config.get("RENTAL")[:400]), "2023-01-12", "2023-01-31"
    )
    ...
    for index, i in enumerate(config.get("RENTAL")):
        ...
        if index < 400:
            ...
            return_date = datetime.datetime.strptime(
                rental_date, "%Y/%m/%d"
            ) + datetime.timedelta(days=random.randint(1, 21))
        last_updated = datetime.datetime.now()
        bookworm_id = (
            config.get("BOOKWORM")[random.randint(0, len(config.get("BOOKWORM")) - 1)]
        ).get("uuid")
        inventory_id = (
            config.get("INVENTORY")[random.randint(0, len(config.get("INVENTORY")) - 1)]
        ).get("uuid")
        staff_id = (
            config.get("STAFF")[random.randint(0, len(config.get("STAFF")) - 1)]
        ).get("uuid")

        if index < 400:
            sql = (
                "INSERT INTO rental (rental_id,rental_date,return_date,last_updated,bookworm_id,inventory_id,staff_id) "

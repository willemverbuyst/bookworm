...
import { Payment, SortDirection } from "../../models";

export const shouldFetchPayments = filter(
  ({ state }: Context) => !state.payment.getAllApi?.data.length
);

export const fetchPayments = async ({ actions, effects, state }: Context) => {
...
export const setPaymentsPage = ({ state }: Context) => {
  state.app.currentPage = Page.PAYMENTS;
};

export const shouldLoadPayments = filter(({ state }: Context) => {
...
export const resetAmountFilter = ({ state }: Context) => {

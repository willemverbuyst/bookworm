...
export const setReviewsPage = ({ state }: Context) => {
  state.app.currentPage = Page.REVIEWS;
};

export const setAddReviewPage = ({ state }: Context) => {
  state.app.currentPage = Page.REVIEWS_NEW;
};

export const getReviews = async ({ actions, effects, state }: Context) => {
  const limit = 3;
  const page = 1;
...
export const getBooksForAuthor = async (
  { actions, effects, state }: Context,

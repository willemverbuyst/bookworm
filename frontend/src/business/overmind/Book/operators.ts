...
export const shouldLoadBooks = filter(({ state }: Context) => {
  return !state.book.getAllApi?.data.length;
});

export const getBooks = async ({ actions, effects, state }: Context) => {
  const {
...
export const resetLanguage = ({ state }: Context) => {
...
export const resetGenre = ({ state }: Context) => {
...
export const getGenres = ({ actions }: Context) => {
  actions.genre.getGenres();
};

export const getLanguages = ({ actions }: Context) => {

...
export const shouldLoadBookworms = filter(({ state }: Context) => {
  return !state.bookworm.getAllApi?.data.length;
});

export const getBookworms = async ({ actions, effects, state }: Context) => {
  const {
    filter: { active },
    limit,
    page,
...
export const resetActiveFilter = ({ state }: Context) => {
